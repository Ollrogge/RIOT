...
#define DTLS_CKXEC_LENGTH (1 + 1 + \
                           max(DTLS_EC_KEY_SIZE + DTLS_EC_KEY_SIZE, \
...
{
    unsigned char buf[DTLS_HMAC_MAX] = { 0 };
...
#ifdef DTLS_FIDO
    hmac_context_t _ctx;
    fido2_ctap_crypto_hmac_sha256_init(&_ctx, ctx->cookie_secret, DTLS_COOKIE_SECRET_LENGTH);
    fido2_ctap_crypto_hmac_sha256_update(&_ctx, (unsigned char *)&session->addr, session->size);
#else
...
#ifdef DTLS_FIDO
    fido2_ctap_crypto_hmac_sha256_update(&_ctx, msg + DTLS_HS_LENGTH, e);
#else
...
#ifdef DTLS_FIDO
    fido2_ctap_crypto_hmac_sha256_update(&_ctx, msg + DTLS_HS_LENGTH + e, fragment_length - e);
    fido2_ctap_crypto_hmac_sha256_final(&_ctx, buf);
#else
    dtls_hmac_update(&hmac_context,
...
static int verify_ext_sig_hash_algo(uint8 *data, size_t data_length)
{
    ...
    if (i + sizeof(uint16) != data_length) {
        dtls_warn(
            "the list of the supported signature_algorithms should be tls extension length - 2\n");
        return dtls_alert_fatal_create(DTLS_ALERT_HANDSHAKE_FAILURE);
    }
                                 ...
                                 uint8 *data, size_t data_length)
{
    ...
    p = dtls_set_handshake_header(DTLS_HT_HELLO_VERIFY_REQUEST, &(ephemeral_peer->mseq),
                                  data_length, 0,
                                  data_length, p);
                             ...
                             int add_hash)
{
    ...
    dtls_set_handshake_header(header_type, &(peer->handshake_params->hs_state.mseq_s), data_length,
                              0,
                              data_length, buf);
                ...
                size_t buf_len_array[], size_t buf_array_len)
{
    ...
    res = dtls_prepare_record(peer, security, type, buf_array, buf_len_array, buf_array_len,
                              sendbuf, &len);
               ...
               uint8 **cleartext)
{
    ...
    dtls_security_parameters_t *security =
        dtls_security_params_read_epoch(peer, dtls_get_epoch(header));
    int clen;
    ...
    if ((rlen = is_record(msg, msglen))) {
        ...
        if (DTLS_CT_HANDSHAKE == content_type && 0 == epoch) {
            ...
            if (hs_header->msg_type == DTLS_HT_CLIENT_HELLO) {
                ...
                dtls_ephemeral_peer_t ephemeral_peer = { session, dtls_uint48_to_int(
        ...
        if (!security) {
            ...
            if (security->cseq.bitfield == 0) { /* first message of epoch */
                ...
                if (seqn_diff == 0) {
                    /* already seen */
                    dtls_debug(
                        "Drop: duplicate packet arrived (cseq=%" PRIu64 " bitfield's start)\n",
                        pkt_seq_nr);
                    return 0;
                }
                    ...
                    if (data_length > 0) {
                        ...
                        dtls_debug(
                            "update bitfield is %" PRIx64 " keep sequence base %" PRIx64 "\n",
                            security->cseq.bitfield, security->cseq.cseq);
                    }
                }
